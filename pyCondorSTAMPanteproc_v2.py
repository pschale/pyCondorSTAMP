...
quit_program, jobs, commentsToPrintIfVerbose, job_groups, jobDuplicates, H1_jobs, L1_jobs = parse_jobs(rawData, quit_program)
...
if not quit_program:
    ...
    if options.anteproc_mode:
        ...
        if jobs["constants"]["anteprocParamsH"]["doDetectorNoiseSim"] == "false":
            anteproc_H_name = anteproc_dir + "/H1-anteproc_params.txt"
            anteproc_L_name = anteproc_dir + "/L1-anteproc_params.txt"
            save_anteproc_paramfile(anteproc_H, anteproc_H_name, anteprocDefaultData)
            save_anteproc_paramfile(anteproc_L, anteproc_L_name, anteprocDefaultData)
        else:
            H_jobs_simulated = []
            L_jobs_simulated = []
            for job in jobs:
        ...
        anteproc_grand_stochtrack_values = [["anteproc.loadFiles", True],
                                            ...
                                            ["anteproc.inmats1", anteproc_dir + "/H-H1_map"],
                                            ["anteproc.inmats2", anteproc_dir + "/L-L1_map"],
                                            ["anteproc.jobfile", newAdjustedJobPath]]
        for job in jobs:
            for line in anteproc_grand_stochtrack_values:
                jobs[job]["grandStochtrackParams"]["params"] = nested_dict_entry(jobs[job]["grandStochtrackParams"]["params"], line[0], line[1])
    else:
        anteproc_dir = None
