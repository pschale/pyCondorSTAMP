...
targetIndex = findSNRIndex(rankedSNRs, options.snr)
targetFAP = ranks[targetIndex]
backgroundSNR = rankedSNRs[targetIndex]

plt.grid(b=True, which='minor',color='0.85',linestyle='--')
plt.grid(b=True, which='major',color='0.75',linestyle='-')
plt.plot(rankedSNRs, ranks,'b-')#, label = "SNR distribution")
plt.plot(options.snr, targetFAP, 'ro', label = "Estimated minimum FAP = " + str(targetFAP) + "%\nTrigger SNR = " + str(options.snr) + "\n(Background SNR = " + str(backgroundSNR) + ")")
#plt.yscale('log')
#plt.xscale('log')
plt.xlabel("SNR")
...
#plt.show()
plt.savefig(options.outputDir + "/SNRvsFAPforTriggerSNRof" + str(options.snr).replace(".","dot"), bbox_inches = 'tight')
plt.clf()
