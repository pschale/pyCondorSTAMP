...
parser.add_option("-u", "--unrestrictedTitle", dest="unrestrictedTitle",
                  help = "Option to set key label for unrestricted data (separate by underscores for spaces)")
parser.add_option("-r", "--restrictedTitle", dest="restrictedTitle",
                  help = "Option to set key label for restricted data (separate by underscores for spaces)")
...
plt.grid(b=True, which='major',color='0.75',linestyle='-')
if options.dots:
    plt.plot(sortedAllSNRs, all_percentage,'b.-', label = options.unrestrictedTitle.replace("_"," "))#, label = "SNR distribution")
    plt.plot(sortedAllSimulatedSNRs, allSimulated_percentage,'g.-', label = options.restrictedTitle.replace("_"," "))#, label = "Monte Carlo Simulations")
else:
    plt.plot(sortedAllSNRs, all_percentage,'b-', label = options.unrestrictedTitle.replace("_"," "))#, label = "SNR distribution")
    plt.plot(sortedAllSimulatedSNRs, allSimulated_percentage,'g-', label = options.restrictedTitle.replace("_"," "))#, label = "Monte Carlo Simulations")
if eventSNRs:
